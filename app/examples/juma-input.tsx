...
const ADVANCED_MODELS: JumaModel[] = [
  { id: 'gpt-6-astra', name: 'GPT-6 Astra', desc: 'Frontier reasoning and code', advanced: true },
...
function ModelGlyph({ id, className }: { id: string; className?: string }) {
  ...
  if (id === 'gpt-6-astra') {
    return <OpenAiMark className={cn('text-[#0d0d0d] dark:text-white', className)} />
  }
  if (id === 'opus-5') {
...
// OpenAI's knot, Anthropic's wordmark "A" (from the product's own model button)
// and Gemini's gradient spark — used as the Advanced models' vendor marks. Each
// carries its brand color; OpenAI's mark is monochrome, so it takes black on
// light and white on dark, the way style-logo.tsx renders it.
...
const ADVANCED_MODELS: JumaModel[] = [
  { id: 'gpt-6-astra', name: 'GPT-6 Astra', desc: 'Frontier reasoning and code' },
